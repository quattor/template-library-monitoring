...
#
#
template monitoring/lemon/sensor/fmonagent;
...
"/system/monitoring/sensor/MSA/class" = list( 
	...
	nlist("name", "MSA.SensorCheck",
		...
		"field", list(
			nlist("name", "UnresponsiveSensors", "format", "%ld", "descr", "The number of sensors that were unresponsive to the agents CHK request"),
			nlist("name", "CheckedSensors", "format", "%ld", "descr", "The number of sensors checked"),
		)
	),
	...
	nlist("name", "MSA.SensorVersions",
		"descr", "Returns the versions of the sensor/modules running under the agent",
		"field", list(
			nlist("name", "SensorName",  "format", "%64s", "primary", true),
			nlist("name", "ModuleName",  "format", "%64s", "primary", true),
			nlist("name", "Version",     "format", "%15s"),
		)
	),	
);
